...
import {
  Terminal, Mail, ArrowRight, ChevronDown,
  User, FolderOpen, Briefcase, Cpu, Globe, X,
  Folder, FileText, ChevronRight, Power,
} from 'lucide-react';
import { useTyping } from '../hooks/useTyping';
import { GithubIcon, LinkedinIcon } from './icons';
import { stats, projects as allProjects, experience as allExp, skills as allSkills } from '../data';
...
const BOOT_LINES: BootLine[] = [
  { text: '> LULIANO_OS v2.0.26 — Initializing...',     color: 'text-violet-400',  pause: 320 },
  { text: '',                                             color: '',                 pause: 100 },
  { text: '[BOOT]  Loading kernel .............. OK',    color: 'text-gray-300',    pause:  80 },
  { text: '[MEM ]  RAM 16GB check .............. OK',    color: 'text-gray-300',    pause:  80 },
  { text: '[GPU ]  Renderer online ............. OK',    color: 'text-gray-300',    pause:  80 },
  { text: '[NET ]  Secure connection ........... OK',    color: 'text-emerald-400', pause: 120 },
  { text: '[AI  ]  Neural interface ............ ACTIVE', color: 'text-cyan-400',  pause: 280 },
  { text: '',                                             color: '',                 pause:  80 },
  { text: '════════════════════════════════════════',    color: 'text-gray-500',    pause: 160 },
  ...
  { text: '  React · Node.js · React Native · Python',  color: 'text-gray-300',    pause: 160 },
  { text: '> status',                                    color: 'text-violet-300',  pause: 220 },
  { text: '  [■■■■■■■■■■] 100% — Ready to build',      color: 'text-emerald-400', pause:   0 },
];

/* ─────────────────────────────── types ─────────────────────────────── */

type Phase  = 'idle' | 'splash' | 'fading' | 'boot' | 'loading' | 'desktop' | 'shutdown';
type AppId  = 'about' | 'projects' | 'skills' | 'experience' | 'contact' | 'terminal' | 'files';
type TermLine = { kind: 'in' | 'out'; text: string };

/* ─────────────────────────────── desktop apps ─────────────────────────── */

interface AppDef { id: AppId; label: string; Icon: React.ComponentType<{className?:string}>; color: string; title: string }

const APPS: AppDef[] = [
  { id: 'about',      label: 'À propos',  Icon: User,       color: 'text-violet-400',  title: 'about.txt'  },
  { id: 'projects',   label: 'Projets',   Icon: Globe,      color: 'text-cyan-400',    title: 'Projets'    },
  { id: 'skills',     label: 'Skills',    Icon: Cpu,        color: 'text-emerald-400', title: 'Skills'     },
  { id: 'experience', label: 'Exp.',      Icon: Briefcase,  color: 'text-amber-400',   title: 'Expérience' },
  { id: 'contact',    label: 'Contact',   Icon: Mail,       color: 'text-pink-400',    title: 'contact.txt'},
  { id: 'terminal',   label: 'Terminal',  Icon: Terminal,   color: 'text-gray-300',    title: '~ terminal' },
  { id: 'files',      label: 'Fichiers',  Icon: FolderOpen, color: 'text-blue-400',    title: 'Fichiers'   },
];
...
function processCmd(raw: string): string {
  ...
  switch (cmd) {
    ...
    case 'cat': {
      ...
      if (f === 'about.txt')
        return ['Profil: fullstack · produit · design · IA',
                'Stack: React, Node.js, React Native, Python',
                'École: ESM Bénin — SIL',
                'Dispo: Freelance & collaborations'].join('\n');
      if (f === 'contact.txt')
        return ['GitHub : github.com/luliano-sondjo',
                'Lieu   : Cotonou, Bénin',
                'Status : Disponible · Freelance'].join('\n');
      if (f === 'README.md')
        return ['# Portfolio — Luliano Sondjo',
                'Dev Web & IA · Vibe Coding',
                "5 ans d'expérience · 18 ans",
                'Built with React + Tailwind'].join('\n');
      if (f === 'dev.txt')
    ...
    case 'projects':
      return allProjects.slice(0, 4).map(p =>
  ...
  return (
    <div className="absolute inset-0 lg:hidden overflow-hidden pointer-events-none flex items-center justify-center px-8">
      <pre className="code-typewriter-bg font-mono text-[0.7rem] sm:text-xs leading-relaxed select-none">
        ...
        {idx < MOBILE_CODE.length && <span className="code-typewriter-cursor">█</span>}
      </pre>
    </div>
  );
}
...
function TuxIcon() {
  return (
    <svg width="80" height="96" viewBox="0 0 80 96" fill="none" aria-hidden="true">
      ...
      <ellipse cx="11" cy="66" rx="10" ry="20" fill="#1e1e2e" transform="rotate(-15 11 66)" />
      <ellipse cx="69" cy="66" rx="10" ry="20" fill="#1e1e2e" transform="rotate(15 69 66)"  />
    ...
    <div className="space-y-2 text-[10px] font-mono">
      ...
      <div className="border-t border-white/5 pt-2">
        <p className="text-gray-300 leading-relaxed">
          Fullstack orienté produit, design & automatisation.
          React · Node.js · React Native · Python.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-2 text-[10px] font-mono">
      {allProjects.slice(0, 4).map((p, i) => (
        <div key={i} className="border border-white/5 rounded-lg p-2 hover:border-violet-500/20 transition-colors">
          <p className="text-white font-semibold mb-0.5">{p.title}</p>
          <p className="text-gray-300 leading-relaxed text-[9px] mb-1 line-clamp-2">{p.desc}</p>
          <div className="flex flex-wrap gap-1">
            {p.tags.slice(0, 3).map((t, j) => (
              <span key={j} className="px-1 py-0.5 rounded bg-white/5 text-cyan-400 text-[8px]">{t}</span>
            ))}
          </div>
        </div>
      ))}
        ...
        <div key={i}>
          ...
          {cat.items.map((s, j) => (
            <div key={j} className="flex items-center gap-2 mb-0.5">
              <span className="text-gray-300 w-24 shrink-0 truncate">{s.name}</span>
              <div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden">
                <div className="h-full bg-violet-400/60 rounded-full skill-mini-fill" style={{ '--w': `${s.level}%` } as React.CSSProperties} />
              </div>
...
function ExperienceContent() {
  return (
    <div className="space-y-2 text-[10px] font-mono">
      {allExp.map((e, i) => (
        <div key={i} className={`pl-2 border-l-2 ${e.current ? 'border-violet-500' : 'border-white/10'}`}>
          ...
          <div className="flex flex-wrap gap-1 mt-0.5">
            {e.tags.slice(0, 3).map((t, j) => (
              <span key={j} className="text-[8px] text-gray-400">{t}</span>
            ))}
          </div>
        </div>
      ))}
...
function ContactContent() {
  return (
    <div className="space-y-2 text-[10px] font-mono">
      ...
      {[
        { label: 'GitHub',  value: 'github.com/luliano-sondjo', color: 'text-gray-300' },
        { label: 'Lieu',    value: 'Cotonou, Bénin',            color: 'text-gray-300' },
        { label: 'Status',  value: 'Disponible · Freelance',    color: 'text-emerald-400' },
        { label: 'Section', value: '#contact sur ce portfolio',  color: 'text-cyan-400' },
      ].map((row, i) => (
...
function TerminalApp() {
  ...
  function submit() {
    ...
    if (out === '__clear__') {
      setLines([{ kind: 'out', text: "LULIANO_OS terminal v2.0 — type 'help'" }, { kind: 'out', text: '' }]);
    } else {
      setLines(prev => [
        ...prev,
        { kind: 'in',  text: `$ ${cmd}` },
  ...
  return (
    <div className="h-full flex flex-col bg-[#050510] cursor-text" onClick={() => inputRef.current?.focus({ preventScroll: true })}>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 space-y-0.5 text-[9px] font-mono">
        {lines.map((l, i) => (
          <div key={i} className={l.kind === 'in' ? 'text-violet-300' : 'text-gray-300 whitespace-pre'}>
            {l.text || ' '}
          </div>
        ))}
      </div>
...
type FileNode = { name: string; type: 'file'; content: string } | { name: string; type: 'dir'; children: FileNode[] };

const FILE_TREE: FileNode[] = [
  { name: 'about.txt',   type: 'file', content: 'Profil: fullstack · produit · design · IA\nStack: React, Node.js, React Native, Python\nÉcole: ESM Bénin — SIL\nDispo: Freelance & collaborations' },
  { name: 'contact.txt', type: 'file', content: 'GitHub : github.com/luliano-sondjo\nLieu   : Cotonou, Bénin\nStatus : Disponible · Freelance' },
  { name: 'README.md',   type: 'file', content: '# Portfolio — Luliano Sondjo\nDev Web & IA · Vibe Coding\n5 ans d\'expérience · 18 ans\nBuilt with React + Tailwind' },
  { name: 'projects', type: 'dir', children: [
    { name: 'e-commerce.md',   type: 'file', content: '# E-Commerce Platform\nNext.js · TypeScript · Stripe\nPostgreSQL · Redis' },
    { name: 'ai-chat.md',      type: 'file', content: '# AI Chat Application\nReact · Python · WebSocket\nOpenAI · Docker' },
    { name: 'saas-dashboard.md', type: 'file', content: '# SaaS Dashboard\nReact · D3.js · Node.js\nMongoDB · AWS' },
    { name: 'mobile-app.md',   type: 'file', content: '# Mobile Fitness App\nReact Native · Firebase\nMapBox · HealthKit' },
  ]},
  { name: 'skills', type: 'dir', children: [
    { name: 'dev.txt',          type: 'file', content: 'HTML/CSS 90%   JavaScript 85%\nReact 82%      Node.js 76%' },
    { name: 'mobile-ia.txt',    type: 'file', content: 'React Native 70%   Expo 70%\nPython IA 25%      Chatbots 60%' },
    { name: 'design-tools.txt', type: 'file', content: 'Figma 78%   Git/GitHub 85%\nLinux 80%   VS Code 90%' },
  ]},
];
  ...
  function renderTree(nodes: FileNode[], depth = 0) {
    return nodes.map(node => (
      <div key={node.name}>
        <button
          type="button"
          onClick={() => node.type === 'dir' ? toggle(node.name) : setSelected(node.name)}
        ...
        >
          {node.type === 'dir' ? (
            <>
              <ChevronRight className={`w-2.5 h-2.5 shrink-0 transition-transform ${expanded.has(node.name) ? 'rotate-90' : ''}`} />
        ...
        {node.type === 'dir' && expanded.has(node.name) && renderTree(node.children, depth + 1)}
      </div>
    ));
  }
  ...
  return (
    <div className="h-full flex gap-1">
      <div className="w-2/5 overflow-y-auto border-r border-white/5 pr-1">
        <p className="text-[8px] text-gray-300 font-mono px-1 py-0.5">/home/luliano/portfolio</p>
        {renderTree(FILE_TREE)}
      </div>
      <div className="flex-1 overflow-y-auto">
        {content ? (
          <pre className="text-[9px] font-mono text-gray-300 leading-relaxed whitespace-pre-wrap p-1">{content}</pre>
        ) : (
          <p className="text-[9px] text-gray-400 font-mono p-2">Cliquez sur un fichier pour l'ouvrir</p>
        )}
      </div>
    </div>
  );
    ...
    <div className="h-full flex flex-col bg-[#0a0a18] border border-white/10 rounded-lg overflow-hidden">
      <div className="flex items-center justify-between px-2 py-1 bg-[#07070f] border-b border-white/5 shrink-0">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[9px] font-mono text-gray-300">{def.title}</span>
        </div>
        <button type="button" aria-label="Fermer" onClick={onClose} className="text-gray-400 hover:text-gray-100 transition-colors">
...
function TaskbarIcon({ label, Icon, color, isActive, onClick }: Omit<AppDef, 'title'> & { isActive: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <Icon className={`w-[15px] h-[15px] ${color} transition-transform group-hover:scale-110`} />
      {isActive && (
        ...
        {openApp ? (
          ...
          <div className="h-full flex flex-col items-center justify-center gap-2">
            <p className="text-gray-300 text-[9px] font-mono text-center leading-relaxed">
              Cliquez sur une icône<br />pour explorer le portfolio
            </p>
            <div className="flex gap-1.5">
              {[0,1,2].map(i => <div key={i} className={DOT_CLS[i]} />)}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="h-9 bg-[#04040e]/95 border-t border-white/8 flex items-center px-3 shrink-0 relative">
        ...
        <div className="absolute inset-x-0 flex justify-center pointer-events-none">
          <div className="flex items-center gap-0.5 pointer-events-auto">
            {APPS.map(app => (
              <TaskbarIcon key={app.id} {...app} isActive={openApp === app.id} onClick={() => toggle(app.id)} />
            ))}
          </div>
        </div>
...
function OsBootTerminal() {
  ...
  useEffect(() => {
    ...
    if (progress >= 100) { setPhase('fading'); return; }
    const delay = progress < 40 ? 18 : progress < 75 ? 28 : progress < 92 ? 45 : 70;
  ...
  const currentLine = phase === 'boot' && lineIdx < BOOT_LINES.length ? BOOT_LINES[lineIdx] : null;
    ...
    <div className="relative bg-[#0c0c16] rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
      ...
      <div className="flex items-center gap-2 px-4 py-3 bg-[#07070f] border-b border-white/5">
        ...
        <span className="text-xs text-gray-300 font-mono ml-2">
          {phase === 'desktop'  ? 'LULIANO OS — Desktop'
            : phase === 'loading'  ? 'LULIANO OS — Démarrage...'
            : phase === 'shutdown' ? 'LULIANO OS — Arrêt...'
            : phase === 'idle'     ? 'LULIANO OS — Éteint'
            : 'LULIANO_OS — terminal'}
        </span>
      </div>
      ...
      <div className="relative">
        ...
        {phase === 'idle' && (
          <div className="min-h-[340px] flex flex-col items-center justify-center gap-4 bg-[#0c0c16]">
            ...
            <div className="text-center">
              <p className="font-mono text-white text-sm font-bold tracking-[0.2em]">LULIANO OS</p>
              <p className="font-mono text-gray-300 text-[9px] tracking-widest mt-0.5">Linux 6.17 LTS — x86_64</p>
            </div>
        ...
        {(phase === 'splash' || phase === 'fading') && (
          <div className={`min-h-[340px] flex flex-col items-center justify-center bg-[#0c0c16] transition-opacity duration-500 ${phase === 'fading' ? 'opacity-0' : 'opacity-100'}`}>
            <TuxIcon />
            <p className="font-mono text-white text-sm font-bold tracking-[0.2em] mt-5 mb-1">LULIANO OS</p>
            <p className="font-mono text-gray-300 text-[10px] tracking-widest mb-8">Linux 6.17 LTS — x86_64</p>
          ...
          <div className="p-5 font-mono text-[13px] min-h-[340px] transition-opacity duration-300">
            ...
            {currentLine && currentLine.text !== '' && (
              <div className={`leading-6 ${currentLine.color}`}>
                {typing}<span className="text-violet-400 animate-pulse">█</span>
              </div>
            )}
            {bootDone && (
          ...
          <div className="min-h-[340px] flex flex-col items-center justify-center gap-5 bg-[#0c0c16]">
            <div className="relative w-16 h-16">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-[10px] font-mono font-bold text-violet-400">LS</span>
              </div>
            </div>
            <div className="text-center space-y-1.5">
              <p className="text-white text-sm font-mono font-bold tracking-[0.28em]">CHARGEMENT</p>
              <p className="text-gray-300 text-[9px] font-mono tracking-[0.15em] animate-pulse">
                Démarrage du bureau en cours...
              </p>
            </div>
            <div className="flex gap-2">
              {DOT_CLS.slice(0, 4).map((cls, i) => <div key={i} className={cls} />)}
            </div>
          </div>
        )}
        ...
        {phase === 'shutdown' && (
          <div className="min-h-[340px] flex flex-col items-center justify-center gap-5 bg-[#0c0c16]">
            ...
            <div className="text-center space-y-1.5">
              <p className="text-white text-sm font-mono font-bold tracking-[0.28em]">ARRÊT DU SYSTÈME</p>
              <p className="text-gray-300 text-[9px] font-mono tracking-[0.15em] animate-pulse">
            ...
            <div className="space-y-[5px] w-56">
              {SHUTDOWN_LINES.slice(0, shutdownStep).map((line, i) => (
                <p key={i} className={`font-mono text-[10px] leading-5 ${line.color}`}>
                  {line.text}
                  {i === shutdownStep - 1 && shutdownStep < SHUTDOWN_LINES.length && (
          ...
          <div>
            ...
            <div className={`lg:hidden mt-6 ${enter} enter-d-800 ${up}`}>
              ...
              >
                ...
                <span className="relative text-gray-300 text-sm font-mono">Lancer LULIANO OS</span>
