...
// ── Remplis ces 3 valeurs depuis dashboard.emailjs.com ──
const EMAILJS_SERVICE_ID  = 'YOUR_SERVICE_ID';
const EMAILJS_TEMPLATE_ID = 'YOUR_TEMPLATE_ID';
const EMAILJS_PUBLIC_KEY  = 'YOUR_PUBLIC_KEY';

const contactInfo = [
  { icon: <Mail className="w-5 h-5" />, label: 'Email', value: 'alex@dupontdev.fr' },
  { icon: <MapPin className="w-5 h-5" />, label: 'Localisation', value: 'Paris, France' },
  { icon: <Globe className="w-5 h-5" />, label: 'Portfolio', value: 'alexdupont.dev' },
];
  ...
  return (
    <section id="contact" className="py-24 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <SectionTitle sub="contact" title="Travaillons Ensemble" desc="Un projet en tête ? Discutons-en !" />
          ...
          <div className={`transition-all duration-700 ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            <div className="space-y-5 sm:space-y-6 mb-8 sm:mb-10">
              <p className="text-gray-300 text-sm sm:text-base md:text-lg leading-relaxed">
                ...
                Que ce soit pour un projet freelance, un poste à temps plein ou simplement
                échanger sur la tech, n'hésitez pas à me contacter !
              </p>
          ...
          <div className={`transition-all duration-700 delay-200 ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
            <form ref={formRef} onSubmit={handleSubmit} noValidate className="glass-card rounded-2xl p-5 sm:p-7 lg:p-8 border-gradient">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-400 mb-1.5">Sujet</label>
                  <input
                    type="text"
                    name="subject"
                    value={form.subject}
                ...
                >
                  ...
                  {status === 'success' && <><CheckCircle2 className="w-5 h-5" /> Message envoyé !</>}
